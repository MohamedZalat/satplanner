...
public final class ASP extends AbstractStateSpacePlanner {
    ...
    */
    public static void main(String[] args) {
        ...
        long parsing_time = info.getTimeToParse();
        long encoding_time = info.getTimeToEncode();
        long searching_time = info.getTimeToSearch();

        // Print time information
        long time = info.getTimeToParse() +  info.getTimeToEncode() + info.getTimeToSearch();
        Planner.getLogger().trace(String.format("%ntime spent:   %8.2f seconds parsing %n", parsing_time/1000.0));
        Planner.getLogger().trace(String.format("              %8.2f seconds encoding %n", encoding_time/1000.0));
        Planner.getLogger().trace(String.format("              %8.2f seconds searching%n", searching_time/1000.0));
        Planner.getLogger().trace(String.format("              %8.2f seconds total time%n", time/1000.0));
                

        long memory_used_problem = info.getMemoryUsedForProblemRepresentation();
        long memory_used_search = info.getMemoryUsedToSearch();
        // Print memory usage information
        long memory = info.getMemoryUsedForProblemRepresentation() + info.getMemoryUsedToSearch();
        Planner.getLogger().trace(String.format("%nmemory used:  %8.2f MBytes for problem representation%n", memory_used_problem/(1024.0*1024.0)));
